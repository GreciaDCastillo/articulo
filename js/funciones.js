document.addEventListener("DOMContentLoaded", function () {
    ...
    if (btnAceptar4) {
        btnAceptar4.addEventListener("click", function () {
            // Acción después de aceptar la advertencia
    ...
    window.addEventListener('message', function (event) {
        ...
        } else if (event.data === 'nextScenario') {
            const iframe = document.querySelector('.escenario-iframe');
    ...
    // Agregar el evento para el botón que transicionará los escenarios
    const btnAceptarTransicion = document.querySelector(".btn-aceptar");
    if (btnAceptarTransicion) {
        btnAceptarTransicion.addEventListener("click", function () {
            mostrarEscenarios();
        });
    }
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (acceptButton) {
            acceptButton.addEventListener('click', function() {
                window.parent.postMessage('nextScenario', '*');
            });
        }
    });
